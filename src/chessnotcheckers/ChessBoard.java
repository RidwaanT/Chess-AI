...
/* I used a lot of references when it came to building the chessboard to build an idea of what to do
*/
import java.util.*;
...
/*  important things to watch or the piece Type which are actually Integers, &
also the color to let us know which team the piece is on
*/
public class ChessBoard {
    public byte[][] board = new byte[8][8]; // using a double array for the board
    public int previousDoublePush;
    public int turn; // this will be represented by a colour
    public boolean[] kingside = new boolean[2], queenside = new boolean[2]; // used for castling
	...
	public SquareDesc() { }
	public SquareDesc(int t, int c, int ix, int iy) { type=t; color=c; 
        x=ix; y=iy; }
    ...
    /** Our chess board is based on the first letter of each piece except for knight which as an n. */
    ...
    /**  Returns a new ChessBoard that's the
	...
	for (int x=0; x<8; x++) {
		...
		pieces.addFirst(new SquareDesc(pieceof(board[x][y]), 
                        colorof(board[x][y]),x,y));
	    }
	}
	...
	// if we have acastling move
	if ((pieceof(board[m.srcx][m.srcy]) == KING) && 
	    ((m.srcx-m.destx >= 2) || (m.srcx-m.destx <= -2))) {
	    makeCastlingMove(m);
	    return;
	}
	// this represents pawn capturing
		//the if looks to see if there is a piece to capture
	...
	/* We invalidate castling whenever we move a
	...
	// Tells us if black or white is making the move
    public int getTurn() {
	return turn;
    }
	...
	if (isempty(board[x][y]))
	    ...
	    return new SquareDesc(pieceof(board[x][y]), colorof(board[x][y]), 
                    x , y);
    }
    
    /* check attacks for pawns*/
	...
	return false;
   	...
   	/* check attacks for all other pieces */
	...
	for (int dir = 0; dir < pdesc[piece].delta.length; dir++) {
		...
		/* cx and cy are represented by the current positions */
		/* if I'm off the board or sitting on a friendly piece, 
                break out */
	...
	if (isempty(board[x][y+dy])) {
	    ...
	    if (!newp.inCheck()) moveList.addFirst(new Move(x, y, x, y+dy, 
                    false));
	    /* push 2 */
	    if (((turn == WHITE && y == 1) || (turn == BLACK && y == 6)) && 
                    isempty(board[x][y+2*dy])) {
		newp.board[x][y+2*dy] = newp.board[x][y+dy];
		newp.board[x][y+dy] = EMPTY;	    
		if (!newp.inCheck()) moveList.addFirst(new Move(x, y, x, 
                        y+2*dy,false));
	    }
	}
	/* captures */
	for (dx = -1; dx <= 1; dx += 2) {
		...
		if (!((turn == WHITE && y == 4) || (turn == BLACK && y == 3))) 
                    continue;
		ex = x+dx;
		ey = y;
	    }
	    ...
	    if (!newp.inCheck()) moveList.addFirst(new Move(x, y, x+dx, y+dy, 
                    true));
	}
    }
	...
	for (dir = 0; dir < pdesc[piece].delta.length; dir++) {
		...
		/* if I'm off the board or sitting on a friendly piece, break 
                out */
		if (cx < 0 || cy < 0 || cx >=8 || cy >= 8) break;
		if (!isempty(board[cx][cy])) {
		    if (colorof(board[cx][cy]) == turn) break;  /* hit a 
                    friendly piece */
		    /* hit an enemy piece */
		    capture = true;
		}
		...
		newp.board[x][y] = EMPTY;
		if (!newp.inCheck()) moveList.addFirst(new Move(x, y, cx, cy, 
                        capture));
    ...
    */
	...
	if (newp.inCheck()) return;
	moveList.addFirst(new Move(4, castleRank, 2, castleRank, false, true));	
    }
	...
	if (newp.inCheck()) return;
	moveList.addFirst(new Move(4, castleRank, 6, castleRank, false, true));	
    }
    ...
    /* The following function generates all the legal moves from the given 
    position. */
